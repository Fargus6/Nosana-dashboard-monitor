...
import { Plus, Trash2, RefreshCw, Activity, ExternalLink, Edit2, Check, X, Eye, EyeOff, LogOut, Moon, Bell, BellOff, Settings, TrendingUp } from "lucide-react";
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <CardTitle className={theme.text.primary + " flex items-center gap-2"}>
                      ...
                      Live Earnings (Scraped from Dashboard)
                    </CardTitle>
                    <p className={theme.text.secondary + " text-sm mt-1"}>
                      {selectedNodeStats.name || "Unnamed Node"}
                    </p>
              ...
              <CardContent className="space-y-4">
                {liveEarningsLoading ? (
                  <div className="text-center py-8">
                    ...
                    <p className={theme.text.secondary}>Scraping live data from Nosana dashboard...</p>
                    <p className={theme.text.muted + " text-xs mt-2"}>This may take a few seconds</p>
                  </div>
                ) : liveEarningsData ? (
                  <>
                    {/* Summary Section */}
                    <div className={"p-4 rounded-lg " + (currentTheme === "cyber" ? "bg-[#00ff00]/20 border-2 border-[#00ff00]" : currentTheme === "neon80s" ? "bg-emerald-500/20 border-2 border-emerald-500" : "bg-green-100 border-2 border-green-500")}>
                      <h3 className={theme.text.primary + " text-lg font-semibold mb-3"}>Summary</h3>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                          <p className={theme.text.muted + " text-xs"}>Total Jobs</p>
                          <p className={theme.text.primary + " text-2xl font-bold"}>{liveEarningsData.summary.total_jobs}</p>
                        </div>
                        <div>
                          <p className={theme.text.muted + " text-xs"}>Completed</p>
                          <p className={theme.text.primary + " text-2xl font-bold text-green-600"}>{liveEarningsData.summary.completed_jobs}</p>
                        </div>
                        <div>
                          <p className={theme.text.muted + " text-xs"}>Total USD</p>
                          <p className={theme.text.primary + " text-2xl font-bold"}>${liveEarningsData.summary.total_usd.toFixed(2)}</p>
                        </div>
                        <div>
                          <p className={theme.text.muted + " text-xs"}>Total NOS</p>
                          <p className={theme.text.primary + " text-2xl font-bold"}>{liveEarningsData.summary.total_nos.toFixed(2)}</p>
                        </div>
                      </div>
                      <p className={theme.text.muted + " text-xs mt-3"}>
                        NOS Price: ${liveEarningsData.nos_price?.toFixed(4) || "N/A"}
                      </p>
                    </div>
                    ...
                    <div className="space-y-2">
                      <h3 className={theme.text.primary + " text-lg font-semibold border-b pb-2"}>
                        Recent Jobs (from Nosana Dashboard)
                      </h3>
                      <div className="space-y-2 max-h-96 overflow-y-auto">
                        {liveEarningsData.jobs && liveEarningsData.jobs.map((job, idx) => (
                          <div 
                            key={job.job_id || idx} 
                            className={"p-3 rounded-lg " + (job.status === "RUNNING" 
                              ? (currentTheme === "cyber" ? "bg-[#00f0ff]/10 border border-[#00f0ff]/30" : currentTheme === "neon80s" ? "bg-cyan-500/10 border border-cyan-500/30" : "bg-blue-50 border border-blue-300")
                          ...
                          >
                            <div className="flex justify-between items-start mb-2">
                              <div className="flex-1">
                                ...
                                <p className={theme.text.secondary + " text-xs mt-1"}>
                                  {job.started_text || "N/A"} • {job.duration_text || "N/A"}
                                </p>
                              </div>
                              <div className="text-right">
                                <span className={
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-2 text-xs">
                              <div>
                                <span className={theme.text.muted}>GPU:</span>
                                <span className={theme.text.primary + " ml-1 font-semibold"}>{job.gpu_type}</span>
                              </div>
                              <div>
                                <span className={theme.text.muted}>Rate:</span>
                                <span className={theme.text.primary + " ml-1 font-semibold"}>${job.hourly_rate_usd}/h</span>
                              </div>
                              <div>
                                <span className={theme.text.muted}>USD Earned:</span>
                                <span className={theme.text.primary + " ml-1 font-semibold"}>${job.usd_earned?.toFixed(4) || "0"}</span>
                              </div>
                              <div>
                                <span className={theme.text.muted}>NOS Earned:</span>
                                <span className={theme.text.primary + " ml-1 font-semibold"}>{job.nos_earned?.toFixed(2) || "0"}</span>
                              </div>
                            </div>
                          </div>
                        ))}
                    ...
                    {/* Info Note */}
                    <div className={"p-3 rounded-lg text-xs " + (currentTheme === "cyber" ? "bg-[#00f0ff]/10 border border-[#00f0ff]/30" : currentTheme === "neon80s" ? "bg-teal-500/10 border border-teal-500/30" : "bg-blue-50 border border-blue-200")}>
                      <p className={theme.text.primary + " font-semibold mb-1"}>ℹ️ About Live Earnings</p>
                      <p className={theme.text.secondary}>
                        This data is scraped directly from the Nosana dashboard in real-time. 
                        It shows actual payment rates and earnings for each job, which may vary based on market conditions.
                      </p>
                    </div>
                  </>
                ) : (
